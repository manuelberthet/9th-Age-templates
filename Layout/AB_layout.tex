...
\newcommand{\invocprofilestart}{%
	...
	\textbf{\labels@profile} &%
	\textbf{\labels@M} & \textbf{\labels@WS} & \textbf{\labels@BS} & \textbf{\labels@S} & \textbf{\labels@T} & \textbf{\labels@W} & \textbf{\labels@I} & \textbf{\labels@A} & \textbf{\labels@Ld} & \textbf{\labels@Invocation} &%
	&%
	\textbf{\labels@trooptype} &%
	\textbf{\labels@basesize}%
}
...
% Unit's big frame.
\tikzset{unitprice/.style={draw=black, fill=white, rectangle, rounded corners, right, minimum height=0.7cm}}
...
\tikzset{unitsize/.style={draw=black, fill=white, rectangle, rounded corners, right, minimum height=0.7cm}}
...
\newenvironment{unitframe}[2][]{%
	\mdfsetup{%
		nobreak=true,%
			  ...
			  singleextra={
				\expandafter\ifstrequal\expandafter{\unit@cost}{-}{}{% STILL WORK IN PROGRESS NEED MACROS AND MORE IF CASE (solo model with additional models)
					\node[unitprice,anchor=east,xshift=-0.5cm] at (P)%
						{%
							\large\pts{\textbf{\unit@cost}}%
							\expandafter\ifblank\expandafter{\unit@costpermodel}{}{ + \textbf{\unit@costpermodel{}} p.a.m.}%
						};
					\node[unitsize, anchor=east, xshift=-4cm] at (P)%
					{%
						\expandafter\ifnumequal\expandafter{\unit@unitsize}{1}{%single model
							\large{\textbf{single} model}%
						}{%not single model					
							\large{\textbf{\unit@unitsize}\expandafter\ifblank\expandafter{\unit@additionalmodels}{}{, up to \textbf{\unit@additionalmodels{}}}  models}%
						}%
					};
				}%
				\node[unittitle,xshift=2.5cm] at (P-|O)%
					 {\LARGE\antiquefont\uppercase\expandafter\expandafter\expandafter{\unit@name}};
				\node[unitlogo, xshift=0.5cm, yshift=0.1cm] at (P-|O)%
